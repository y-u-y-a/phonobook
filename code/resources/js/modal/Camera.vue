<template>
  <div @click="issueTrigger" id="modal-camera" class="modal-default text-center">
    <!-- カメラによる動画の挿入 -->
    <video ref="video" width="100%" height="60%" class="mb-4" autoplay muted></video>
    <!-- 切り取った画像を描画 -->
...
export default {
  ...
  methods: {
    ...
    setCamera() {
      ...
      // カメラの起動
      navigator.mediaDevices.getUserMedia(camera_config).then(stream => {
    ...
    async authFace() {
      ...
      // 本人データの取得
      await axios.post("/api/users/authFace", params).then(response => {
